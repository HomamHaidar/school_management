            ...
            <div class="card card-statistics h-100">
                <div class="card-body">
                    ...
                    @endif
                        ...
                        <button type="button" class="btn btn-outline-success btn-lg"  data-toggle="modal" data-target="#exampleModal"> {{ trans('My_Classes_trans.add_class') }}</button>
                        <button type="button" class="btn btn-outline-danger  btn-lg" id="btn_delete_all">{{ trans('My_Classes_trans.delete_checkbox') }}</button>

                    <br><br>
                            <form action="{{ route('Filter_Classes') }}" method="POST" >
                                {{ csrf_field() }}
                                <select class=" selectpicker btn btn-outline-info"  name="Grade_id" required
                                        onchange="this.form.submit()">
                                    <option style="background-color: white; color: black" value=""  selected disabled>{{ trans('My_Classes_trans.Search_By_Grade') }}</option>
                                    @foreach ($Grades as $Grade)
                                        <option style="background-color: white; color: black"  value="{{ $Grade->id }}">{{ $Grade->Name }}</option>
                                    @endforeach
                                </select>
                            </form>
                        <br>
                    <div class="table-responsive">
                        <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                            ...
                            @if (isset($details))
                                <?php $List = $details; ?> {{--if we choice filter from the controller we send details in this case the foreach should show just about fillter --}}
                             @else
                                <?php $List =$Classes; ?>{{--here we have the normal case with all classes --}}
                            @endif

                            <?php $i = 0; ?>

                            @foreach ($List as $My_Class)
                                <tr>
                                    <?php $i++; ?>
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form action="{{ route('classrooms.destroy', 'test') }}"
                                                    ...
                                                    {{ method_field('Delete') }}
                                                    @csrf
                                                    {{ trans('My_Classes_trans.Warning_Grade') }} <br>
                                                    <input class="form-control" name="Name" id="Name" type="text" readonly value="{{$My_Class->Name_Class}}">
                                                    <input id="id" type="hidden" name="id" class="form-control"
                                                           value="{{ $My_Class->id }}">
                                                    <div class="modal-footer">
                                                        <button type="button" class="btn btn-secondary"
